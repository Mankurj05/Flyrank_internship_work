...
- Name: primary-flow.spec.ts >> Weather Assistant Primary Flow >> loads the application and shows empty state
...
Locator: getByRole('heading', { name: 'Weather Assistant' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: 'Weather Assistant' })

```

```yaml
- banner: Weather Assistant
- main:
  - heading "How can I help you today?" [level=2]
  - paragraph: Try asking for the weather in a specific city, e.g., "What's the weather like in Tokyo?"
  - button "\"What's the weather like in Tokyo?\""
  - button "\"Get weather for New York in celsius\""
  - paragraph: (To test the error state, include "error" in the location)
- contentinfo:
  - textbox "Ask about the weather..."
  - button [disabled]
