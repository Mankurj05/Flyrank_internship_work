...
- Name: primary-flow.spec.ts >> Weather Assistant Primary Flow >> clicks example prompt and fills input
- Location: e2e\primary-flow.spec.ts:18:7

# Error details

```
Error: locator.click: Error: strict mode violation: getByText('What\'s the weather like in Tokyo?') resolved to 2 elements:
    1) <p class="mt-2 max-w-sm text-sm sm:text-base">Try asking for the weather in a specific city, e.…</p> aka getByText('Try asking for the weather in')
    2) <button class="rounded-lg border border-zinc-300 bg-white px-4 py-2 text-sm text-zinc-700 hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-800">"What's the weather like in Tokyo?"</button> aka getByRole('button', { name: '"What\'s the weather like in' })

Call log:
  - waiting for getByText('What\'s the weather like in Tokyo?')
...
- generic [active] [ref=e1]:
  - generic [ref=e2]:
    ...
    - contentinfo [ref=e20]:
      - generic [ref=e21]:
        - generic [ref=e22]:
          - textbox "Ask about the weather..." [ref=e23]
